...
app.get("/api/people", function (_, res) {
    return contacts_1.default.find({}).then(function (results) { return res.json(results); });
});
//get by id
app.get("/api/people/:id", function (_a, res, next) {
    var id = _a.params.id;
    contacts_1.default.findById(id)
...
app.put("/api/people/:id", function (req, res, next) {
    ...
    contacts_1.default.findByIdAndUpdate(req.params.id, person, {
        new: true,
        runValidators: true,
        context: "query",
    })
...
app.delete("/api/people/:id", function (_a, res, next) {
    var id = _a.params.id;
    contacts_1.default.findByIdAndRemove(id)
        .then(function (result) { return res.status(204).end(); })
        .catch(function (err) { return next(err); });
});
//add from body
app.post("/api/people/", function (_a, res, next) {
    ...
    person
        .save()
        .then(function (savedContact) { return res.json(savedContact); })
        .catch(function (err) { return next(err); });
});
//get info
app.get("/info", function (_, res) {
    var contacts = contacts_1.default.find({}).then(function (results) {
        res.send("Phonebook has info for ".concat(results.length, " people").concat(new Date().toLocaleString()));
    });
});
...
app.listen(PORT, function () { return console.log("Server runnong on port ".concat(PORT)); });
